...
function PrerenderSPAPlugin(...args) {
    ...
    this._options.renderer = this._options.renderer || new PuppeteerRenderer(Object.assign({}, { headless: true, args: ["--no-sandbox"] }, rendererOptions));
...
PrerenderSPAPlugin.prototype.apply = function (compiler) {
    ...
    const afterEmit = (compilation, done) => {
        ...
        PrerendererInstance.initialize()
            ...
            .then((renderedRoutes) => (this._options.postProcess ? Promise.all(renderedRoutes.map((renderedRoute) => this._options.postProcess(renderedRoute))) : renderedRoutes))
